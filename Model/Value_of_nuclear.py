...
if single_year_index == -1:
    for year_cycle_idx in range(3):
        ...
        for idx in range(len(co2_constraints_list)):
            ...
            for idx_cap in range(len(capacity_specified)):
                tech_list[nuclear_idx]['capacity'] = capacity_specified[idx_cap] / 1.0
                case_dic['case_name'] = f'{case_name_default}_Year{str(year)}_Co2Con{str(co2_constraints_percentage[idx])}_NuCap{str(capacity_specified[idx_cap])}'
    ...
    for idx in range(len(co2_constraints_list)):
        ...
        for idx_cap in range(len(capacity_specified)):
            tech_list[nuclear_idx]['capacity'] = capacity_specified[idx_cap] / 1.0
            case_dic['case_name'] = f'{case_name_default}_Year{str(single_year_index)}_Co2Con{str(co2_constraints_percentage[idx])}_NuCap{str(capacity_specified[idx_cap])}'
            run_model_main_fun(case_dic, tech_list) 